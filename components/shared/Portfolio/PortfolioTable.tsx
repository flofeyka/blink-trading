...
export default function PortfolioTable() {
        ...
        const fetchPositions = async () => {
            try {
                ...
                setPositions(positions.map(position => ({
                    ...position, avg_price: {
                        price_token: position.avg_price, price_usd: position.avg_price_usd
                    }, price: {
                        price_token: position.price, price_usd: position.price_usd
                    }, balance: {
                        balance_ui_token: position.balance_ui, balance_ui_usd: position.balance_ui_usd
                    }, pnl: {
                        pnl_usd: position.pnl_usd, pnl_percent: position.pnl_percent
                    }
                })));
            } catch(e) {
                console.error(e);
    ...
    const columns: Column<any>[] = [{
        ...
        sortable: true,
        minWidth: "170px",
        render: (_, row) => (<div className="flex items-center gap-2">
        ...
        align: "left",
        minWidth: "130px",
        render: (value) => (<div className="flex flex-col ">
        ...
        align: "right",
        minWidth: "130px",
        render: () => (<div className="flex justify-end gap-2 items-center">
                <Image
                    src="/icons/share.svg"
                    width={25}
                    height={25}
        ...
        align: "right",
        minWidth: "180px",
        render: (value) => (<div className="flex justify-end gap-2 items-center">
                <Button onClick={() => onWithdraw(value)}
    ...
    return (<div>
            <div
                className="flex md:justify-between md:items-center pb-3 border-b border-[#353535] max-md:flex-col max-md:gap-3">
                <div className="flex items-center gap-5 overflow-x-auto whitespace-nowrap pb-2">
                    {tabs.map((tab) => (<span
                            key={tab.id}
                            className={`cursor-pointer flex items-center gap-2 ${activeTab === tab.id ? "font-medium max-md:text-md" : "text-[#A9A9A9] font-medium max-md:text-md"}`}
          ...
          <span className="flex items-center gap-2">
            ...
            <div className="mt-2">
                {loading ? <div className={'text-center h-[150px] flex items-center justify-center text-[#716F7A]'}>Loading...</div> : <Table
                    data={positions}
                    columns={columns}
